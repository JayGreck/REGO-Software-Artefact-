...
async function retireREGOS(holder, id, burnAmount, accreditor) {
  burn = await REGORegister.retireREGOCertificate(holder, id, burnAmount, accreditor);
  await burn.wait();
}

async function mintREGOS(holder, id, burnAmount, accreditor) {
  burn = await REGORegister.retireREGOCertificate(holder, id, burnAmount, accreditor);
  await burn.wait();
}

async function transferREGOS(holder, id, burnAmount, accreditor) {
  burn = await REGORegister.retireREGOCertificate(holder, id, burnAmount, accreditor);
  await burn.wait();
}
...
async function main() {
   ...
   if (await REGORegister.getREGOApplicant(PUBLIC_KEY) == false) {
    ...
    await mint.wait();
    try {
      // const burn = await REGORegister.retireREGOCertificate(PUBLIC_KEY, 12, 4000, PUBLIC_KEY);
      // await burn.wait();
      ...
      // await transfer.wait();
      const balance = await REGORegister.balanceOf(PUBLIC_KEY, 4);
      console.log(balance);
    } catch(err) {
      console.log(err);
    }
